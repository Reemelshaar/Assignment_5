# train_kaggle.py
"""
Kaggle-ready training script for SeqTrack (Assignment 5)

- Uses current folder as repo root
- Fixes Windows path issues
- Ensures lib imports work
- Uses fallback dummy model if SeqTrackV2 not available
...
SEQTRACKV2_PATH = PROJECT_ROOT  # Kaggle repo root
...
try:
    ...
    print(f"[WARNING] Could not import TrainingLogger: {e}")
    class TrainingLogger:
        def __init__(self, log_path, print_every=50):
            self.print_every = print_every
        def start_epoch(self, epoch, total_samples): pass
        def step_samples(self, n): pass
        def log_metrics(self, epoch, phase, **kwargs): pass
...
def load_checkpoint(path, model, optimizer=None, scheduler=None, map_location="cpu", restore_rng=True):
    ...
    if restore_rng:
        try:
            if "rng_python_state" in ckpt: random.setstate(ckpt["rng_python_state"])
            if "rng_numpy_state" in ckpt: np.random.set_state(ckpt["rng_numpy_state"])
            if "rng_torch_state" in ckpt: torch.set_rng_state(ckpt["rng_torch_state"])
...
def parse_args():
    ...
    parser.add_argument("--subset_json", type=str, default="dataset/subset_info.json")
...
def import_seqtrack_class():
    ...
    return None, "No SeqTrack class found"
